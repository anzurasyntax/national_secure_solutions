<section id="blog" class="mx-auto max-w-[92%] px-4 py-14 pb-24 scroll-mt-24 sm:max-w-[88%] lg:max-w-[80%] lg:py-16 lg:pb-40">
    <!-- Header -->
    <div class="mb-12 text-center">
        <p class="font-heading text-sm font-semibold uppercase tracking-[3px] text-[#cc0000]">Our Blog</p>
        <h2 class="font-heading text-3xl font-bold text-[#1c1c25] sm:text-[38px]">Latest Articles</h2>
        <span class="mx-auto mt-4 block h-[3px] w-20 bg-[#cc0000]"></span>
        <p class="mt-6">
            <a href="{{ route('blog') }}" class="inline-flex items-center gap-2 font-heading text-base font-semibold uppercase tracking-wide text-[#cc0000] underline decoration-2 underline-offset-4 hover:no-underline sm:text-lg">
                View all posts
        ...
        @forelse (($blogPosts ?? collect()) as $blogPost)
            <article class="service-card overflow-hidden bg-white shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
                ...
                <div class="p-6">
                    ...
                    @endif
                    <h3 class="mb-3 font-heading text-xl font-bold leading-snug text-[#1c1c25] md:text-[28px]">
                        <a href="{{ route('blog.show', $blogPost->slug) }}" class="hover:text-[#cc0000]">{{ $blogPost->title }}</a>
                    </h3>
                    @if ($blogPost->excerpt)
                        <p class="mb-4 text-base leading-relaxed text-gray-500 sm:text-lg">{{ \Illuminate\Support\Str::limit($blogPost->excerpt, 140) }}</p>
                    @endif
                    <a href="{{ route('blog.show', $blogPost->slug) }}" class="flex items-center gap-1 font-heading text-base font-semibold text-[#cc0000] underline hover:no-underline sm:text-lg">
                        Read More
                    </a>
                </div>
            </article>
